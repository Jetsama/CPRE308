...
int PRAlgo_FIFO(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
	...
	int oldest_arrival = PageFrames[0].time_of_arrival; 

	for(int i = 0; i<num_frames; i++){
		PageFrame frame =PageFrames[i];
		if(frame.time_of_arrival < oldest_arrival ){
			frame_to_evict = i; 
			oldest_arrival  = frame.time_of_arrival;
		}
	}
...
{
	int frame_to_evict = 0;
	int last_used = PageFrames[0].time_of_access; 

	/*TODO: fill in the code below */
	for(int i = 0; i<num_frames; i++){
		PageFrame frame =PageFrames[i];
		if(frame.time_of_access < last_used ){
			frame_to_evict = i; 
			last_used  = frame.time_of_access;
		}
	}
...
{
	int frame_to_evict = 0;
	int longest_access = 0; 
	/*TODO: fill in the code below */

	for(int i = 0; i<num_frames; i++){
		PageFrame frame =PageFrames[i];
		int next_access = PageAccesses[current_access];
		int access = current_access;
		while(next_access != frame.page_id && access <=num_accesses){
			access++;
			next_access = PageAccesses[access];
			//printf("%d\n",next_access);
		}
		if(access > longest_access){
			longest_access = access;
			frame_to_evict = i;
		}
	}
